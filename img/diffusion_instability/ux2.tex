...
% \pgfplotstableread{data.tsv}\mytable
	...
	\begin{tikzpicture}
		...
		\addplot[name path = A, domain={-3:3},samples=1000,thick,blue!50!magenta]  ({x+1.2*exp(-2*x^2)}, {exp(-x^2)});
		\path[name path=axis] (axis cs:-3,0) -- (axis cs:3,0);
		\draw[fill=white] (1.13,0.5) circle (1.3pt);
    ...
    fill between[
		...
		% \draw[->] (0,{exp(-0^2)}) -- (2,{exp(-0^2)});
		% \draw[->] (0.5,{exp(-0.5^2)}) -- (1.5,{exp(-0.5^2)});
		% \draw[->] (0.8,{exp(-0.8^2)}) -- (1.3,{exp(-0.8^2)});
		% \draw (1,0) circle (2.5);
