...
% \pgfplotstableread{data.tsv}\mytable
	...
	\begin{tikzpicture}
		...
		% \xdef\w{(pi+pi/4)}
		\addplot[thick,black, domain={-1.5*pi:1.5*pi},samples=1000] {sqrt(x^2+1)};

		\addplot[dashed, domain={-1.5*pi:1.5*pi},samples=1000] {sqrt(x^2)} node [below,pos=0.75,sloped] {$\omega=\pm vk$};
